...
class DatabaseService {
  ...
  open() async {
    ...
    database = await openDatabase(path, version: 1, onCreate: (Database db, int version) async {
      ...
      await db.execute('CREATE TABLE Stock (id INTEGER PRIMARY KEY, name TEXT, price DOUBLE, stock DOUBLE)');
    });
  }
  ...
  insert(SqliteModel model) async {
    await database?.transaction((txn) async {
      await txn.rawInsert('INSERT INTO Stock(name, price, stock) VALUES(?, ?, ?)', [model.name, model.price, model.stock]);
    });
  }

  query() async {
    if (database != null) {
      List<Map> list = await database!.rawQuery('SELECT * FROM Stock order by id desc');
      return list;
    }
  }
